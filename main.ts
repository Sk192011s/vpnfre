...
const CYCLE_START_DAY = parseInt(Deno.env.get('CYCLE_START_DAY') || '1');
...
const GENERATE_RATE_WINDOW = 60 * 1000;
...
// FIX: Use a Set for faster lookups and thread-safe operations
let activeGeneratedUUIDs: Set<string> = new Set();

async function refreshActiveUUIDs(): Promise<void> {
  try {
    const store = await loadKeysStore();
    activeGeneratedUUIDs = new Set(store.keys.map(k => k.uuid.toLowerCase()));
    console.log(`Refreshed active generated UUIDs: ${activeGeneratedUUIDs.size} keys`);
  } catch (e) {
    console.error('Failed to refresh active UUIDs:', (e as Error).message);
    // Don't clear existing UUIDs on error - keep serving existing connections
  }
}

await refreshActiveUUIDs();

// Refresh periodically (every 1 minute instead of 5 for better sync)
setInterval(async () => {
  await refreshActiveUUIDs();
}, 1 * 60 * 1000);
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .cycle-date-item { text-align: center; }
        ...
        .generate-section { margin: 32px 0; }
        ...
        .btn-generate .icon { font-size: 1.2rem; }
...
Deno.serve(async (request: Request) => {
  ...
  if (url.pathname === '/api/generate' && request.method === 'POST') {
    ...
    // FIX: Immediately add to active set (lowercase for consistent matching)
    activeGeneratedUUIDs.add(newUUID.toLowerCase());
    console.log(`New key generated. UUID: ${maskUUID(newUUID)}. Total active generated: ${activeGeneratedUUIDs.size}`);
    ...
    .pipeTo(
      new WritableStream({
        async write(chunk, controller) {
          ...
          // FIX: Get ALL valid UUIDs at the time of connection (fresh read)
...
function processVlessHeader(vlessBuffer: ArrayBuffer, validUserIDs: string[]) {
  ...
  let incomingUUID: string;
  try {
    incomingUUID = stringify(new Uint8Array(vlessBuffer.slice(1, 17))).toLowerCase();
  } catch (_) {
    return { hasError: true, message: 'invalid UUID in header' };
  }
  ...
  for (const id of validUserIDs) {
    if (constantTimeEqual(id, incomingUUID)) {
      isValidUser = true;
      break; // FIX: break early once found (optimization, won't affect constant-time per comparison)
    }
  }
